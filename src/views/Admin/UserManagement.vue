...
const openEditModal = (user: User) => {
  ...
  form.value = {
      ...
      no_hp: user.no_hp !== '-' ? (user.no_hp || '') : '',
      role: user.role === 'mahasiswa' ? 'mahasiswa' : 'staff',
...
const handleSubmit = async () => {
  if (!form.value.name || !form.value.username) return alert("Nama & Username wajib diisi!")

  if (!isEditing.value && !form.value.password) return alert("Password wajib untuk user baru!")
  ...
  try {
    ...
    if (form.value.role === 'mahasiswa') {
        url = isEditing.value
            ? `http://localhost:8000/api/admin/mahasiswa/${form.value.id}`
            : `http://localhost:8000/api/admin/mahasiswa`
        payload.prodi = form.value.prodi
        payload.id_dosen_pa = form.value.id_dosen_pa
    } else {
        url = isEditing.value
            ? `http://localhost:8000/api/admin/staff/${form.value.id}`
            : `http://localhost:8000/api/admin/staff`
    ...
    <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      <div v-else v-for="user in filteredUsers" :key="user.id" class="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5 items-center hover:bg-gray-50 dark:hover:bg-meta-4 transition">
        ...
        <div class="col-span-2 sm:col-span-2 flex justify-center">
          <span :class="`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium border uppercase ${getRoleBadgeColor(user.role)}`">{{ user.role }}</span>
        </div>
        <div class="col-span-1 sm:col-span-1 flex flex-col sm:flex-row items-center justify-center gap-2">
          <button @click="openEditModal(user)" class="text-blue-600 hover:underline text-xs">Edit</button>
          <button @click="handleDelete(user)" class="text-red-600 hover:underline text-xs">Hapus</button>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="fixed inset-0 z-[99999] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
        <div class="w-full max-w-lg rounded-lg bg-white p-8 shadow-2xl dark:bg-boxdark max-h-[90vh] overflow-y-auto">
          <h3 class="mb-6 text-xl font-bold text-black dark:text-white">{{ isEditing ? 'Edit User' : 'Tambah User' }}</h3>

          <form @submit.prevent="handleSubmit">
            <div v-if="!isEditing" class="mb-4">
                <label class="mb-2 block text-sm font-medium text-black dark:text-white">Tipe User</label>
                <div class="flex gap-4">
                    <label class="flex items-center cursor-pointer"><input type="radio" v-model="form.role" value="mahasiswa" class="mr-2"> Mahasiswa</label>
                    <label class="flex items-center cursor-pointer"><input type="radio" v-model="form.role" value="staff" class="mr-2"> Staff / Dosen</label>
                </div>
            </div>

            <div class="mb-4">
                <label class="mb-2 block text-sm font-medium text-black dark:text-white">Nama Lengkap</label>
                <input v-model="form.name" class="w-full rounded border border-stroke p-3 outline-none focus:border-blue-600 dark:bg-form-input dark:border-form-strokedark" />
            </div>
            <div class="mb-4">
                <label class="mb-2 block text-sm font-medium text-black dark:text-white">Username (NIM/NIP)</label>
                <input v-model="form.username" class="w-full rounded border border-stroke p-3 outline-none focus:border-blue-600 dark:bg-form-input dark:border-form-strokedark" />
            </div>

            <div class="mb-4">
                <label class="mb-2 block text-sm font-medium text-black dark:text-white">
                    {{ isEditing ? 'Reset Password (Opsional)' : 'Password' }}
                </label>
                <input v-model="form.password" type="password" class="w-full rounded border border-stroke p-3 outline-none focus:border-blue-600 dark:bg-form-input dark:border-form-strokedark"
                    :placeholder="isEditing ? 'Isi jika ingin mereset password user ini' : 'Minimal 6 karakter'" />
                <p v-if="isEditing" class="text-xs text-slate-500 mt-1 italic">Biarkan kosong jika tidak ingin mengubah password user.</p>
            </div>
            ...
            <div class="mb-4">
                ...
                <input v-model="form.no_hp" type="text" class="w-full rounded border border-stroke p-3 outline-none focus:border-blue-600 dark:bg-form-input dark:border-form-strokedark" placeholder="08xxxxxxxx" />
            </div>

            <template v-if="form.role === 'mahasiswa'">
                <div class="mb-4">
                  <label class="mb-2 block text-sm font-medium text-black dark:text-white">Dosen PA</label>
                  <div class="relative z-20 bg-transparent dark:bg-form-input">
                    <select v-model="form.id_dosen_pa" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent p-3 outline-none focus:border-blue-600 dark:border-form-strokedark dark:bg-form-input">
                      <option value="" disabled>Pilih Dosen</option>
                      <option v-for="dosen in dosenOptions" :key="dosen.id" :value="dosen.id">{{ dosen.nama }}</option>
                    </select>
                    <span class="absolute top-1/2 right-4 z-30 -translate-y-1/2"><ChevronDownIcon class="w-4 h-4"/></span>
                  </div>
                </div>
                <div class="mb-4">
                    <label class="mb-2 block text-sm font-medium text-black dark:text-white">Prodi</label>
                    <div class="relative z-20 bg-transparent dark:bg-form-input">
                        <select v-model="form.prodi" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent p-3 outline-none focus:border-blue-600 dark:border-form-strokedark dark:bg-form-input">
            ...
            <template v-else>
                <div class="mb-4">
                    <label class="mb-2 block text-sm font-medium text-black dark:text-white">Jabatan</label>
                    <div class="relative z-20 bg-transparent dark:bg-form-input">
                        <select v-model="form.jabatan" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent p-3 outline-none focus:border-blue-600 dark:border-form-strokedark dark:bg-form-input">
            ...
            <div class="flex gap-4 justify-end mt-6">
              <button type="button" @click="showModal = false" class="text-black hover:text-slate-700 px-4">Batal</button>
              <button type="submit" :disabled="isSubmitting" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 disabled:opacity-70">

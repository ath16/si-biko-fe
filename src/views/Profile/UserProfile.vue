...
// --- STATE ---
const isPending = ref(false) // Simulasi status pending approval
...
const kontakAkademik = ref([
  ...
  {
    nama: 'Ibu Ratna Dewi, M.Psi., Psikolog',
    peran: 'Konselor Kemahasiswaan',
    nip: '-',
...
// Cek status saat load (Simulasi)
onMounted(() => {
  // Jika ini real app, cek ke API: /api/profile/request-status
  // if (response.status === 'pending') isPending.value = true
})

const handleFileUpload = (event: Event) => {
  if(isPending.value) return alert("Profil sedang dikunci menunggu persetujuan Admin.")
  ...
  if (target.files && target.files[0]) {
    ...
    reader.onload = (e) => {
      ...
      // Disini bisa langsung request update foto juga
    }
...
const handleRequestUpdate = () => {
  ...
  if(confirm("Ajukan perubahan data ke Admin? Profil akan dikunci sementara hingga disetujui.")) {
    // Simulasi kirim data ke API
    console.log("Request Update Sent:", formData.value)
...
<template>
  <div class="mx-auto max-w-242.5">
    ...
    <div v-if="isPending" class="mb-6 rounded-sm border border-yellow-200 bg-yellow-50 p-4 text-yellow-800 dark:bg-yellow-900/20 dark:border-yellow-800 dark:text-yellow-500">
      <div class="flex items-center gap-3">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        <div>
          <h4 class="font-bold">Perubahan Sedang Ditinjau</h4>
          <p class="text-sm">Anda telah mengajukan perubahan data. Profil dikunci sementara hingga Admin menyetujui permintaan Anda.</p>
        </div>
      </div>
    </div>
      ...
      <div class="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
        <div class="relative mx-auto mt-4 h-30 w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:w-44 sm:p-3">
          <div class="relative drop-shadow-2 h-full w-full">
            ...
            <label v-if="!isPending" class="absolute bottom-0 right-0 flex h-8.5 w-8.5 cursor-pointer items-center justify-center rounded-full bg-blue-600 text-white hover:bg-blue-600/90 sm:bottom-2 sm:right-2">
    ...
    <div class="grid grid-cols-1 gap-9 sm:grid-cols-2">
      ...
      <div class="flex flex-col gap-9">
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          <form @submit.prevent="handleRequestUpdate">
            <fieldset :disabled="isPending" class="contents">
              <div class="p-6.5">
                <div class="mb-4.5">
                  <label class="mb-2.5 block text-black dark:text-white">Nama Lengkap</label>
                  <input v-model="formData.fullName" type="text" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-blue-light-500 dark:border-form-strokedark dark:bg-form-input disabled:bg-gray-100 disabled:cursor-not-allowed" />
                </div>
                <div class="mb-4.5">
                  <label class="mb-2.5 block text-black dark:text-white">Nomor Telepon / WhatsApp</label>
                  <input v-model="formData.phoneNumber" type="text" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-blue-light-500 dark:border-form-strokedark dark:bg-form-input disabled:bg-gray-100 disabled:cursor-not-allowed" />
                </div>
                <div class="mb-4.5">
                ...
                <button
                  v-if="!isPending"
                  class="flex w-full justify-center rounded bg-blue-600 p-3 font-medium text-gray-100 hover:bg-blue-600/90 transition"
                >
                ...
                <button
                  ...
                  type="button"
                  disabled
                  class="flex w-full justify-center rounded bg-gray-400 p-3 font-medium text-white cursor-not-allowed"
                >
      ...
      <div class="flex flex-col gap-9">
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          <div class="p-6.5">
            <div class="mb-4.5">
              ...
              <textarea v-model="formData.bio" :disabled="isPending" rows="6" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-blue-light-500 dark:border-form-strokedark dark:bg-form-input disabled:bg-gray-100 disabled:cursor-not-allowed"></textarea>
            </div>
            <div class="flex justify-end">
              <button v-if="!isPending" @click="handleRequestUpdate" class="text-blue-600 hover:underline font-medium text-sm">Simpan Bio</button>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          <div class="p-6.5">
             <p class="text-sm text-slate-500 mb-4">Disarankan mengganti kata sandi secara berkala.</p>
             <button class="flex w-full justify-center rounded border border-stroke p-3 font-medium text-black hover:bg-gray-50 dark:border-strokedark dark:text-white transition">Ubah Kata Sandi</button>
          </div>
        </div>
      </div>
